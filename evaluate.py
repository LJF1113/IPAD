...
parser.add_argument('--model', type=str, default='VST')
...
for k,(imgs) in enumerate(test_batch):
    ...
    imgs = Variable(imgs['batch']).cuda()
    with torch.no_grad():
        outputs = model(imgs)
        # print(torch.max(outputs['recon_index'],axis=1))
        scores,indics = torch.max(outputs['recon_index'],axis=1)
        # print(indics.item())
    ...
    # print(mse_imgs)
    recon_index_list.append(indics.item())
    ...
    if args.img_dir is not None or args.vid_dir is not None:
        ...
        if args.img_dir is not None:
            ...
            cv2.imwrite(os.path.join(args.img_dir, 'GT_{:04d}.png').format(k), saveimgs)
        if args.vid_dir is not None:
...
if args.print_time:
    ...
    print('test_batch_size: ', len(test_batch))
...
#     video_name = video.split('/')[-2]
#     # cur_psnr_list = psnr_list[video_name]
...
for vi, video in enumerate(sorted(videos_list)):
    ...
    cur_psnr_list = psnr_list[video_name]
    # anomaly_score_total_list += score
    score += psnr_list[video_name]
    vid_idx += [vi for _ in range(len(cur_psnr_list))]
anomaly_score_total_list = anomaly_score_list(score)


anomaly_score_total_list = np.asarray(anomaly_score_total_list)
scipy.io.savemat('p_score.mat',mdict={'p_score':anomaly_score_total_list})

accuracy = AUC(anomaly_score_total_list, np.expand_dims(1-labels_list, 0))
print(anomaly_score_total_list.shape)
print((np.expand_dims(1-labels_list, 0)).shape)
plt.figure()
plt.plot(anomaly_score_total_list)
plt.plot(1-labels_list)
plt.savefig('gt_res_prob_list.jpg')
if args.print_score:
